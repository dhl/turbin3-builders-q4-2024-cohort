...
pub fn create_registrar(
    ...
    registrar.community_token_mint = ctx.accounts.community_token_mint.key();
    registrar.community_token_threshold = community_token_threshold;
    ...
    registrar.previous_voter_weight_plugin_program_id = use_previous_voter_weight_plugin
        .then(|| {
            remaining_accounts
                .first()
                .ok_or(ConditionalPluginError::MissingPreviousVoterWeightPlugin)
    ...
    require_eq!(
        ...
        ConditionalPluginError::InvalidRealmAuthority
    );
